...
//            tvTimer2.setText(time.toString())
            // 메인 UI를 건드는 작업!!
            // 직접적으로 view정보를 수정하는게 아니고
            // 메인 작업Queue에
            // ' 이 작업 해주세요 ...' 라고 작업을 추가 !!
        }
    }
    ...
    // 시간 관련 쓰레드 클래스를 만들자!!
    // 온크리에이트 안에 쓰레드를 만들면 쓰레드 시간이 다된후에 실행이 됨 될수 있으면 밖에다 만들기
    inner class TimerThread(val tv:TextView) : Thread(){
        ...
        override fun run() {
            ...
            for(i in 10 downTo 0){
                ...
                val message=Message()
                // member변수 (field)가 3개존재
                //arg1(아규먼트 /전달인자) : int , arg2 : int , obj(오브젝트) : Any
                //arg1에 담아주어 위에서 사용
                message.arg1= i
                message.obj=tv
