...
class BoardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lv.setOnItemClickListener { adapterView, view, i, l ->
            ...
            // view : ListView가 있는 현재 페이지에대한 정보
            // i (position) -> 내가 클릭한 item위치 (index 0~
            // l (id):long -> 내가 클릭한 item의 고유한 값
            if (i == 0) {
                Toast.makeText(
                    this, adapterView.adapter.getItem(i).toString(),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
...
//        ( 단, LoginActivity에서 받아올 결과 값이 있음 : 양방향 )
        ...
        lv.setOnItemClickListener { adapterView, view, i, l ->
            ...
            // AlertDialog 옵션 정보를 담는 builder생성!

            val builder = AlertDialog.Builder(this)
            builder.setTitle("게시글 삭제")
            builder.setMessage("정말 삭제하시겠습니까?")
            builder.setPositiveButton("삭제",
                OnClickListener { dialogInterface, id ->
                    data.removeAt(i)
                    adapter.notifyDataSetChanged()
                })
    ...
    val result = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        ...
        // resultCode상태를 확인 하자 ok, canceled
